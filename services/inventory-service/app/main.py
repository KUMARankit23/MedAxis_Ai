...
setup_sentry("inventory-service")


@asynccontextmanager
async def lifespan(app: FastAPI):
    logger.info("Inventory service starting — initialising database")
    await init_db()
...
async def health():
    ...
    try:
        ...
        return {"service": "inventory-service", "status": "ok", "checks": {"database": "ok"}}
    except Exception as e:
        logger.error(f"Health check failed: {e}")
        from fastapi.responses import JSONResponse
        return JSONResponse(
            status_code=503,
            content={"service": "inventory-service", "status": "degraded", "checks": {"database": "error"}},
        )
