...
from app.routes import get_current_user as inventory_get_current_user  # noqa: E402
...
async def test_create_medicine(client):
    resp = await client.post("/inventory/medicines", json={
        ...
        "unit_price": 2.50, "reorder_level": 50, "reorder_quantity": 200, "unit": "tablets",
    })
...
async def test_receive_batch_and_deduct(client):
    med_resp = await client.post("/inventory/medicines", json={
        ...
        "unit_price": 3.50, "reorder_level": 20, "reorder_quantity": 100,
    })
    ...
    batch_resp = await client.post("/inventory/batches", json={
        "medicine_id": med_id, "batch_number": "BATCH-TEST-001",
        "outlet_id": "OUTLET-001", "quantity": 100, "expiry_date": expiry,
    })
    assert batch_resp.status_code == 201

    deduct_resp = await client.post("/inventory/deduct", json={
        "medicine_id": med_id, "outlet_id": "OUTLET-001",
        "quantity": 30, "reference_id": "TEST-INV-001",
    })
...
async def test_deduct_insufficient_stock(client):
    med_resp = await client.post("/inventory/medicines", json={
        "name": "Rare Medicine", "category": "PRESCRIPTION",
        "unit_price": 100.0, "reorder_level": 5, "reorder_quantity": 20,
    })
    med_id = med_resp.json()["id"]
    resp = await client.post("/inventory/deduct", json={
        "medicine_id": med_id, "outlet_id": "OUTLET-001",
        "quantity": 10, "reference_id": "TEST-FAIL-001",
    })
