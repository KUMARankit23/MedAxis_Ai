...
from app.routes import get_current_user as billing_get_current_user  # noqa: E402
...
async def test_invoice_items_not_empty(client):
    resp = await client.post("/billing/invoices", json={
        "outlet_id": "OUTLET-001", "payment_method": "CASH", "items": [],
    })
...
async def test_invoice_stats(client):
    ...
    assert "invoice_count" in resp.json()
